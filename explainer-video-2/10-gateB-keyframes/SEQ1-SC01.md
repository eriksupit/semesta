...
```json
{
  ...
  "subject_state": "dynamic",
  "action": "the focus racks to his wife Ratna who has just woken and is sitting up on the bed camera-left, lifting her head and pushing upright from the pillow, while Herman, having stepped much closer toward her, looms large in the soft-focus foreground filling the camera-right third of the frame",
  "pose": "Herman standing in profile facing camera-left, stepped much closer to the camera and toward his wife, looming very large as a soft out-of-focus mass filling the camera-right third of the frame; his wife Ratna sitting up on the bed camera-left in sharp focus, more centred in the frame, torso raised off the pillow, one hand on the mattress, hair loose and tousled",
  "expression": "Ratna just-woken, eyes opening heavy-lidded, calm; Herman calm settled morning composure",
...
| SH04 MS samping (profil) | ✅ (profileA-tunggal + lighting-ref) | START ✅ · END ⏳ | **START ✅ ACCEPT** — `sc01_sh04_start.png`; Herman bangkit PROFIL hadap camera-left (profileA-tunggal, front dibuang), MS barefoot, Ratna tidur camera-left, nakas camera-right, subuh gelap, kontinuitas dari `sc01_sh03_end` (lighting-ref). END ⏳ (anchor `sc01_sh04_start`+`R_ratna_front_medium`+`H_herman_profileA_full`+`bed3q`+`master`; rack-focus ke Ratna duduk) |

**GATE B authoring SEQ1-SC01 LENGKAP 4/4** (8 prompt utuh: SH01–SH04 × start/end). NEXT: user generate keyframe → review face/room vs plat → Kling 3.0 i2v (5dtk, 1080p audio-on, dual reference-lock) → VO + AE grafis → assemble → PILOT REPORT.
