...
> **NO framed wall art + NO wall clock (Erik 2026-07-01):** drop the print so rooms do not repeat the Hokusai motif (boring) + drop the wall clock. Back wall = plain matte plaster above the sideboard. E06 EXITS the Hokusai allocation — only E01 Great Wave / E02 Red Fuji / E03 Kajikazawa keep prints (already generated); E05 + E06 are print-free.
...
```json
{
  ...
  "subject_anchor": {
    ...
    "supporting_objects": {
      "sideboard": "IKEA HAVSTA low sideboard buffet, white-stained oak effect, far back wall, plain closed cabinet, otherwise tidy",
      "back_wall": "plain matte plaster wall muted off-white, above the sideboard, unadorned",
...
- **Branded furniture lock-anchors** keep furniture identical across angles — EKEDALEN dining set + HAVSTA sideboard + LILL curtains = house-register IKEA pieces named for strong prior; distinct silhouettes from the other rooms' furniture. Cross-angle consistency now rides on the furniture (the print is gone), which is the stronger anchor anyway.
- **NO framed artwork (Erik 2026-07-01):** drop the Hokusai print so rooms do not repeat the motif (boring) — back wall = plain matte plaster above the sideboard. E06 EXITS the Hokusai allocation; only E01 Great Wave / E02 Red Fuji / E03 Kajikazawa keep prints (already generated).
- **NO wall clock (Erik 2026-07-01):** drop the wall clock from the camera-RIGHT wall.
...
Each cell is a COMPLETE copy-paste-ready prompt, zero scaffold. §1 is the canonical identity-lock; the cells are what you paste. Scoped to E06's plates: **MASTER only** (establishing WIDE, serves SH01 + SH05 finale). **MEJA DROPPED 2026-07-01** — a same-angle tighter table-end MS is a sub-framing, not a new angle, so the upload-master + §1-verbatim derivative method reference-dominated (reproduced the master wide; §1's "four place settings, centre" also fought the "one end" shot). SH02/SH04 (Herman at the table, MS) are framed at the SC13 keyframe stage off the master + Herman; SH03 phone CU crops tight (no env plate). The master alone locks the room → E06 plate library = master-only.

### Cell M — MASTER · establishing WIDE → `E06_ruangmakan_master.png` · ✅ prompt LOCKED · generate PENDING (sesi baru)
Full dining-room geography — the anchor plate + the finale wide. Establishes the 4-chair table centre, sideboard far back wall (plain plaster above), window camera-LEFT, doorway camera-RIGHT.
```json
{
  "mood": "neutral reference clarity, calm empty domestic stillness",
  "color_grade": "neutral true-to-life color, balanced white-point",
  ...
  "shot": {
    "composition": "full dining room geography, one frame, EKEDALEN dining table centered, four matching chairs around the table, clean place settings, empty serving platter centre, low HAVSTA sideboard far back wall, plain matte plaster wall above the sideboard, potted plant on the sideboard camera-RIGHT end, LILL-curtained window camera-LEFT wall, open doorway camera-RIGHT wall, pendant lamp over the table switched off, ceramic floor, legible",
    ...
    "camera_direction": "down the room over the dining table, toward the sideboard, plain plaster back wall, window camera-left, doorway camera-right",
...
### Cell MEJA — ⛔ DROPPED 2026-07-01 (do NOT generate)
**Reason:** a same-angle tighter table-end MS is a sub-framing (zoom), not a new angle — the upload-master + §1-verbatim derivative method reference-dominated (v1 render reproduced the master wide; `_rejected/E06_ruangmakan_meja_v1_rejected_referencedominance.png`). The full §1 `subject_anchor` ("four place settings, centre") also fought the shot-block's "one end". SH02/SH04 (Herman at table, MS) are framed at the SC13 **keyframe** stage off the accepted master + Herman. Prompt below kept for record only — NOT a generate target.
Serves SEQ6-SC13-SH02/SH04 (M Herman at the table + phone, 50mm). One end of the dining table at a seated adult's height, the place setting + chair the focal mid-ground, sideboard behind (plain plaster above).
```json
{
  "environment_reference": "attached reference plate E06_ruangmakan_master.png, same family dining room, identical furniture, identical layout, identical materials, plain plaster back wall, faithful room match, new camera per shot block only",
  "mood": "neutral reference clarity, calm empty domestic stillness",
  "color_grade": "neutral true-to-life color, balanced white-point",
  "style": "photorealistic interior reference plate, architectural photography clarity, ARRI Alexa Mini LF, clean prime spherical lens, Kodak Portra rendering",
  "scene": "family dining room, urban middle-class family home, IKEA-style flat-pack laminate furniture, matte painted plaster walls, plain desaturated textiles, low sideboard, lived-in working-family",
  ...
  "shot": {
    "composition": "medium framing on one end of the dining table, EKEDALEN table edge, one clean place setting, a dining chair at the table-end, dominant centre, low HAVSTA sideboard behind, plain matte plaster wall above the sideboard, compressed back wall, pendant lamp above switched off",
    ...
    "camera_direction": "toward one end of the dining table, the seat, sideboard, plain plaster back wall",
...
|---|---|---|---|---|
| M | establishing WIDE | `E06_ruangmakan_master.png` | SC13-SH01/SH05 (finale) | ✅ DONE — accepted `E06_ruangmakan_master.png` (EKEDALEN 4-chair + HAVSTA sideboard + plant; NO print, NO clock; pendant/device/night-grade = GATE B; neutral plate) |
| MEJA | table-end MS | — | SC13-SH02/SH04 | ⛔ DROPPED 2026-07-01 (reference-dominance + §1 full-table fought the sub-framing; SH02/SH04 framed at keyframe stage off master). E06 = master-only ✅ |

Storage: `environment-images/E06_ruangmakan/` (+ `_rejected/`). Naming: `E06_ruangmakan_<angle>.png`.
Generate M first (no reference-image). Then MEJA via the `shot`-only-rewrite method: upload `E06_ruangmakan_master.png` as reference + paste §1 verbatim + the cell's `shot` block. Verify each prop against the master before locking. No device in the plate; no framed print; no wall clock; phone + UI + warm night grade = GATE B / After Effects.

---

*E06 Ruang Makan environment sheet — Fase-2 prompts LOCKED (generate PENDING, sesi baru) · Explainer Video 2 · 2026-07-01 · INT same house E01–E03 · FINALE convergence (4 figures + food = GATE B) · NO framed print (E06 exits Hokusai allocation, anti-repeat) · NO wall clock · empty plate no-device (A1) · token-per-token · camera-relative direction.*
