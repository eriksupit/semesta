...
### Cell M4 — PRAYER high-angle (soft, edit-in-place) → `E02_ruangkeluarga_prayer_high.png` · ✅ ACCEPTED 2026-07-02
Serves the prayer high-angle / sujud overview (shot slotted at Fase-3). **Method = edit-in-place on the accepted `E02_ruangkeluarga_master.png` render.** Green prayer carpet + PERSISK rolled + cushions off floor, subuh grade held. Pixel-verified vs master: single contiguous carpet, handedness + grade + screens-OFF consistent. ⚠ Angle landed **soft ~25–30°** rather than the specified ~45° (edit-in-place under-executes large vantage jumps) — accepted as a render-reliable soft high-angle, distinct from the eye-level `saf`. Accepted prompt verbatim:
```text
Edit-in-place (canvas = render E02_ruangkeluarga_master):
Raise the camera above standing height near the doorway corner and tilt it down about forty-five degrees onto the open centre floor, a soft high-angle view of the prayer floor looking across the room toward the far back window-desk wall. Keep the same room and handedness exactly: EKTORP sofa and framed Red Fuji on camera-left, BESTÅ TV bench and off television on camera-right, MICKE desk and off desktop pushed to the far back camera-right wall, KALLAX shelf to the left of the sheer-curtained window, open doorway back-left.
Lay one single plain solid muted-green prayer carpet flat across the central ceramic-tiled floor as one clean contiguous textile, its long edge running left-to-right parallel to the back window-desk wall; move the two floor cushions off the floor onto the EKTORP sofa seat; keep the PERSISK rug rolled beside the TV bench on camera-right.
Keep the same subuh grade exactly: warm ceiling-lamp ambient with localized falloff, cool blue-gray dawn bleed from the sheer-curtained back window, neutral off-white plaster walls, natural oak laminate tones, lifted blacks, restrained contrast, no yellow wash, no orange cast, all screens OFF dark neutral. The room stays empty and still, no people. Do not mirror the room, do not move the furniture. Keep 16:9.
```

### Cell M5 — REVERSE prayer, facing-camera (2-step: reference-upload → edit-in-place) → `E02_ruangkeluarga_reverse_prayer.png` · ✅ ACCEPTED 2026-07-02
Serves the facing-camera prayer angle (kiblat = window; camera at the window-desk wall capturing worshippers' faces). **True 180° reverse — edit-in-place from a render CANNOT do this (it under-shoots even 45°), so a 2-step method was used.** Pixel-verified: genuine reverse (doorway wall now the background; window-desk wall behind camera), correct handedness swap (TV bench + rolled PERSISK camera-LEFT, sofa + Red Fuji camera-RIGHT), doorway clean (no void), carpet + rolled rug + cushions-on-sofa preserved, grade + screens-OFF consistent.

**Step 1 — reference-upload `E02_ruangkeluarga_saf.png` → reverse (`saf_reverse`):**
```text
Use the attached E02_ruangkeluarga_saf render as the reference: same living room, identical furniture and materials, single Hokusai Red Fuji, same subuh grade, and the SAME prayer-floor set already staged in it (green prayer carpet laid, PERSISK rolled beside the TV bench, cushions off the floor). Keep that exact carpet size/shape/position and rolled-rug position. NEW reverse camera per this shot only.
Camera: a real 180° reverse camera move (not a mirrored or flipped image). In the saf reference the camera sits on the doorway side looking toward the far back window-desk wall; now move it to the opposite end — at the far back window-desk wall, backing onto the sheer-curtained window and MICKE desk, looking back across the same green prayer carpet toward the front doorway wall ahead. Sides swap: EKTORP sofa and framed Red Fuji on CAMERA-RIGHT; BESTÅ TV bench, off television, and the rolled PERSISK beside it on CAMERA-LEFT; open doorway on the far front wall ahead; KALLAX shelf on a side wall.
Grade identical to saf: warm ceiling-lamp ambient with localized falloff, cool blue-gray pre-dawn ambient now from behind the camera, neutral off-white plaster, natural oak tones, lifted blacks, no yellow wash, all screens OFF. Empty room, no people. Wide establishing, 24mm, eye-level slight low angle, 16:9. Do not produce this by simply flipping the reference left-to-right.
```
**Step 2 — edit-in-place doorway-fix on the step-1 reverse render (canvas = `saf_reverse`) → ACCEPTED (Erik-authored, verbatim):**
```text
Edit-in-place (canvas = render E02_ruangkeluarga_saf_reverse):
Use the attached E02_ruangkeluarga_saf_reverse render as the reference: same living room, identical furniture and materials, single Hokusai Red Fuji print, same prayer-floor staging, same reverse-camera room identity, and same subuh grade direction. Keep the room empty and still.
Main correction: Remove the strange dark inner doorway / secondary doorway visible inside the front opening. Do not place another door, corridor opening, or black vertical void behind the main doorway. The space beyond the main doorway should read as a simple dim adjacent wall or shallow domestic entry area, not a second doorway. Let the back surface behind the opening become a quiet plaster wall with lifted shadow detail.
Doorway architecture: Keep one normal domestic open doorway on the far front wall only. Make it wider and more believable than the previous narrow version, with realistic residential proportions, a painted door frame, visible wall thickness/reveal, jamb shadows, and a clear threshold line. The doorway should not become a tall black portal. The interior beyond should remain dim, but not pure black, with neutral low-key shadow detail and subtle continuation of the ceramic floor. No door leaf blocking the opening.
Composition and handedness: Preserve the real 180° reverse camera view, not a mirrored or flipped image. The camera is positioned at the far back window-desk wall, backing onto the sheer-curtained window and MICKE desk, looking across the green prayer carpet toward the front doorway wall. In this reverse view, the BESTÅ TV bench, off television, and rolled PERSISK rug are on CAMERA-LEFT; the EKTORP sofa with the cushions on its seat and the framed Hokusai Red Fuji print are on CAMERA-RIGHT; the KALLAX shelf sits on the far/front wall area toward camera-right; the wall hook and folded headscarf are on the right side wall near the sofa.
Floor: Keep one single plain solid muted-green prayer carpet laid flat across the central ceramic-tiled floor, broad and contiguous, smooth unpatterned wool, clean plain edges. The carpet dominates the lower half to lower two-thirds of the frame. Keep the rolled PERSISK oriental rug tightly rolled on the floor beside the BESTÅ TV bench on camera-left, pushed close to the TV bench, aligned lengthwise front-to-back, with patterned outer layer visible. Keep the two flat cushions neatly on the EKTORP sofa seat, not on the floor.
Furniture and objects: Keep the same oak-effect BESTÅ TV bench on camera-left with the flat-screen television screen OFF dark neutral. Keep the beige EKTORP two-seat sofa on camera-right. Keep the Red Fuji print correctly oriented on the camera-right wall above the sofa. Keep the 2x2 KALLAX shelf against the front/right wall area with the rolled item and book in the cubes. Keep the wall hook with folded headscarf. Do not add people or extra objects.
Grade and lighting: Match the original subuh grade from E02_ruangkeluarga_saf as closely as possible: warm ceiling-lamp ambient with localized falloff, restrained practical warmth, neutral off-white plaster walls, natural oak laminate furniture tones, lifted blacks, shaped but readable shadows, restrained contrast, no overall yellow wash, no orange cast, no sepia, no catalog-style flat warmth. Cool blue-gray pre-dawn ambient should be subtle and motivated from behind the camera side, not overpowering. All screens remain OFF dark neutral, no glow, no UI.
Framing: Wide establishing reverse shot, 24mm prime spherical, eye-level slight low angle, deep focus, 16:9. Keep the green prayer carpet prominent in the lower frame, ceiling lamp visible near the top, and the front doorway wall across the background. The doorway must sit quietly as background architecture, secondary to the prayer floor.
Negative constraints: Do not mirror or flip the reference. Do not move the sofa, TV bench, Red Fuji print, KALLAX shelf, green prayer carpet, rolled rug, or cushions. Do not add people. Do not add a second inner doorway. Do not create a dark hallway portal inside the doorway. No black void, no horror-like entrance, no narrow slit doorway, no oversized tall doorway, no pure black interior beyond the doorway, no extra door leaf, no glowing television, no glowing desktop monitor, no UI, no daylight, no yellow wall wash.
```
...
| Cell | Angle | File | Status |
|---|---|---|---|
| M4 prayer high-angle | prayer high-angle / sujud overview | `E02_ruangkeluarga_prayer_high.png` | ✅ ACCEPTED 2026-07-02 — edit-in-place from master; green carpet + rolled PERSISK, grade+handedness held, screens OFF; ⚠ soft ~25–30° (edit-in-place under-shot the 45°, accepted as reliable soft high-angle distinct from `saf`); overwrote legacy neutral plate |
| M5 reverse prayer | REVERSE facing-camera prayer (green carpet) | `E02_ruangkeluarga_reverse_prayer.png` | ✅ ACCEPTED 2026-07-02 — true 180° reverse via 2-step (reference-upload `saf` → reverse, then edit-in-place doorway-fix); handedness swapped (TV+rolled-rug camera-left, sofa+Red Fuji camera-right), doorway clean, set+grade held, screens OFF; overwrote legacy `_reverse_prayer.png`. Obsolete siblings `_reverse.png` + `_reverse_prayer_subuh.png` DELETED by Erik 2026-07-02 |
| F | floor-seating | ~~`E02_ruangkeluarga_floorseat.png`~~ | 🗑 DELETED by Erik 2026-07-02 — ngaji scene (SEQ6-SC02) cut, not reused |
