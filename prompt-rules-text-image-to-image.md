...
- **Instruction discipline:** the edit text is **token-per-token** (comma-separated discrete descriptors, no `and/with/but` glue — same SOP as the JSON prompt), stating ONLY the delta + an explicit lock list (`framing unchanged, crop unchanged, head position unchanged, lighting unchanged, skin unchanged, hair unchanged`). Set input-fidelity to its highest setting if offered (UI dropdown Low/Medium/High → High; a UI control, NEVER a prompt token). Keep the delta minimal — an over-long edit instruction reintroduces drift.
- **NO self-reference preamble (validated 2026-06-29, ChatGPT edit UI shown for SH01-END).** The edit box already operates on the image loaded in the canvas — that canvas IS the START render. Do NOT prefix the edit with `edit attached <file>` nor name the START's own filename; it is redundant and adds a stray token the model may try to composite. Open directly with the operation + delta + lock-list (`Change only the eye state to open; expression: …; unchanged: …`). Attach an ADDITIONAL reference ONLY via the editor's `+` control, and ONLY when the edit needs an element not already in the canvas (an orientation/pose plate, Rule 27) — name that added file alone. Single-image small-delta edits carry ZERO filename.
...
- **PHYSICAL-SCENE MODELING + BIG-DELTA HOLDS (validated 2026-06-29, SH02-END, 4 re-rolls)  🔒:** before writing an edit that adds / repositions a body part or object, **model the 3-D scene** — do NOT swap a single word and call it fixed: (a) a limb's **entry edge follows the character's actual position in the frame** (sleeper at camera-left → forearm enters from camera-left, never an arbitrary "from bottom"; Rule 24); (b) a **grasped object changes orientation to match the grasp** (a phone picked up from the side turns portrait→landscape) — state it, or the spec self-contradicts ("horizontal forearm gripping a vertical phone"); (c) limb + grip + object-orientation + lock-list must be **mutually coherent**, never partial. **Big finding:** edit-in-place held framing pixel-faithfully even for a LARGE delta (object reorientation + added forearm) — this **revises the earlier "big-delta → resample framing" hypothesis**: the real cause of past big-delta failures was an INCOHERENT / partial spec, NOT delta size. A coherent delta + `<everything> unchanged` survives big changes. Behavioural root of the 4 re-rolls: "writing tokens without thinking" — emit only after the scene is modelled.
...
- **2026-06-29 — EDIT-IN-PLACE no-filename-preamble (§178) + physical-scene-modeling + big-delta-holds (EDIT-IN-PLACE §Status) + A/B negation N=1.** (1) The edit box operates on the already-loaded canvas → an edit prompt is operation+delta+lock-list ONLY, no `edit attached <file>`/self-filename; name a file only for an EXTRA `+` reference (validated SH01-END). (2) Model the 3-D scene before an add/reposition edit: limb entry-edge follows the character's frame position (Rule 24), a grasped object reorients to the grasp (portrait→landscape), every element mutually coherent; edit-in-place HELD framing for a LARGE delta (object reorient + added forearm) → the old "big-delta resamples framing" hypothesis is revised: incoherent/partial spec was the real cause, not delta size (validated SH02-END, 4 re-rolls). (3) Adding a body part needing identity → attach the character's OWN neutral plate (e.g. `front_full`) via `+` as "skin and hand reference only" + drop ambiguous colour tokens; prefer a neutral full-body plate over a face-CU render (face-bleed). (4) A/B on the new ChatGPT edit-model: `negative_spatial_instructions` ("do not mirror / do not add second hand") did NOT backfire (N=1) — the GPT-Image-2-era no-negation-backfire does not automatically apply to the new edit-in-place model; affirmative `<x> unchanged` kept as default (simpler, long-validated), re-test on a layout-critical case before changing the rule.
