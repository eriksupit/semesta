...
| Image-to-video A | Kling AI Premier — **model 3.0** (draft di 2.6) | Scene dinamis — karakter 2, 3, 5 |
...
|---|---|---|
| Success rate Kling (wajah manusia, scene dinamis) | 50% ⚠️ | Benchmark generasi LAMA — **wajib re-benchmark di Kling 3.0** (reference-locking ke-3–5 gambar diperkirakan menaikkan angka). Verifikasi 10–15 generate uji sebelum dikunci. |
...
| Biaya kredit Kling 3.0 — 1080p image-to-video **dengan audio bawaan** | 12 kredit/detik (60/5 dtk · 120/10 dtk) | Terverifikasi web Juni 2026 (eesel + panduan Kling; silang invideo) |
...
### 2. Kling AI Premier (model 3.0, audio bawaan menyala)
...
**Model & teknik (diperbarui Juni 2026):**
- **Render final: Kling 3.0** — dipilih karena fitur penguncian karakter berbasis 3–5 gambar referensi (Elements / Director Mode), yang TIDAK dimiliki Kling 2.6. Ini menyerang langsung akar masalah konsistensi wajah manusia di scene dinamis.
- **Draft eksplorasi: Kling 2.6** (lebih murah) sebelum prompt/anchor dikunci.
- **Teknik generate: start→end keyframe** — frame awal dan akhir tiap segmen diisi anchor image hasil ChatGPT Image v2; Kling menginterpolasi gerak di antaranya.
- **Audio bawaan Kling: MENYALA** (ambient/efek suara menyatu gerak; VO narasi tetap diproduksi terpisah). Tarif 1080p audio-on = **12 kredit/detik**.
- **Satuan generate: segmen 5 detik** (standar produksi) — per-kredit identik dengan 10 detik (linear, tanpa ongkos-start), tetapi tiap generate gagal hanya membuang 60 kredit (vs 120), memangkas pemborosan iterasi separuh, dan memberi kontrol start→end keyframe lebih ketat.

**Kalkulasi kebutuhan (basis per detik):**

| Scene | Durasi diterima | Segmen 5 dtk | Kredit diterima (×12/dtk) |
|---|---|---|---|
| K2 — Pemuda | 30 dtk | 6 segmen | 360 |
| K3 — Kurir | 30 dtk | 6 segmen | 360 |
| K5 — Pengusaha | 30 dtk | 6 segmen | 360 |
| **Subtotal diterima** | 90 dtk | 18 segmen | **1.080 kredit** |

Koreksi success rate 50% ⚠️ (angka generasi lama — lihat catatan re-benchmark):
90 dtk ÷ 50% = 180 dtk harus digenerate → 180 × 12 = 2.160 kredit
Tambah buffer 20%: 2.160 × 1,2 = **~2.592 kredit dibutuhkan**

**Kapasitas per satu subscribe Premier (8.000 kredit, 1080p audio-on 12 kredit/dtk):**

| Ukuran | Total detik | Setara menit |
|---|---|---|
| KOTOR (semua generate) | 666 dtk | ±11 mnt |
| BERSIH @50% | 333 dtk | ±5,5 mnt |
| BERSIH @65% (estimasi 3.0 + reference-lock) | 433 dtk | ±7 mnt |

Kebutuhan utama hanya **2.592 kredit** (~90 dtk bersih) — satu subscribe Premier (8.000) sudah menutupi produksi utama dengan sisa ~5.400 kredit. Karena itu:
- Subscribe pertama: produksi utama + iterasi (margin sangat besar).
- Subscribe kedua: cadangan revisi scene, shot ulang, penyesuaian pasca-assembly.

> ⚠️ **Catatan re-benchmark:** angka success rate 50% adalah benchmark Kling generasi lama. Dengan reference-locking 3.0, angka riil kemungkinan lebih tinggi → kebutuhan "2× subscribe" berpotensi turun ke 1×. Jalankan 10–15 generate uji Kling 3.0 untuk mengukur success rate aktual sebelum mengunci jumlah subscribe.
