...
| 2026-06-29 | **⭐ MASTER-PARENT + EDIT-IN-PLACE-ANGLE (universal — env, character, scene).** A camera ANGLE change is NOT a fresh full-prompt with references re-uploaded into a new chat window — that resamples and drifts (inconsistent across windows; proven across ~6 `E01_kamar_bed4q` renders, both Claude's and ChatGPT's full prompts). **Canonical:** every scene/asset has ONE MASTER image (parent, one full from-scratch prompt); every other ANGLE is an **EDIT-IN-PLACE child generated in the SAME context window / iteration lane as the master**, by attaching the master annotated **(1) recent camera view** (current) **+ (2) next/goal camera view** (terse goal-delta) — not a full JSON cell. Within a single shot, framing+angle stay LOCKED, only the subject moves START→END (camera-lock). Two edit-in-place uses: (a) between cuts = move the angle; (b) within a shot = subject moves. The model does not reliably act on abstract `camera_position: left/right` tokens; in-lane edit-in-place is what moves the camera. (Validated 2026-06-29, accepted in-lane edit `E01_kamar_bed4q`; Erik conclusion. Canonical: `prompt-rules-text-image-to-image.md` changelog 2026-06-29.) |
